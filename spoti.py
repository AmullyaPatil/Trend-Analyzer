...
INDIA_TOP_URL = "https://api.spotify.com/playlist/37i9dQZF1DX4JAvHpjipBk"
GLOBAL_TOP_URL = "https://api.spotify.com/v1/browse/new-releases"
...
def get_trending_fashion():
    try:
        ...
        return jsonify({"fashion": fashion_items})
    except Exception as e:
        return jsonify({"error": str(e)})
...
def get_trending_news():
    try:
        ...
        data = requests.get(url).json()
        news = [
            {"title": article["title"], "poster": article.get("urlToImage", None)}
...
def get_movie_info(movie_name):
    ...
    try:
        ...
        soup = BeautifulSoup(response.text, 'html.parser')
        review_elements = soup.find_all('span', class_='sc-16c36l0-1 eNlxlZ')
        reviews = [review.get_text() for review in review_elements]
        return jsonify({"reviews": reviews or ["No reviews found."]})
    except Exception as e:
        return jsonify({"error": str(e)})
